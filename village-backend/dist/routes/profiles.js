...
router.get('/:userid', async (req, res) => {
    ...
    try {
        const result = await db_1.default.query('SELECT profileid, userid, profile_picture FROM profiles WHERE userid = $1 LIMIT 1', [userid]);
...
router.put('/:userid/picture', async (req, res) => {
    ...
    try {
        const result = await db_1.default.query(`INSERT INTO profiles (userid, profile_picture)
       VALUES ($1, $2)
       ON CONFLICT (userid)
       DO UPDATE SET profile_picture = EXCLUDED.profile_picture
       RETURNING profileid, userid, profile_picture`, [userid, profile_picture]);
        res.json(result.rows[0]);
    }
    catch (err) {
